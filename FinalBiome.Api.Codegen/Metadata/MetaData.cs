...
public class MetaDataV14
{
    ...
    {
        Origin = origin;
        Magic = HexUtils.Bytes2HexString(rtmd.MetaDataInfo.Magic.Bytes);
        ...
        NodeMetadata = new NodeMetadataV14()
        {
            ...
            TypeId = (uint)rtmd.RuntimeMetadataData.TypeId.Value.Value
        };
    }
        ...
        foreach (var type in types)
        {
            ...
            var typeParams = type.Ty.TypeParams.Value.Length == 0 ? null : type.Ty.TypeParams.Value.Select(p =>
            {
                return new NodeTypeParam()
                {
                    ...
                    TypeId = p.TypeParameterType.Value?.Value.Value
                };
            }).ToArray();
            ...
            switch (typeDefValue)
            {
                ...
                case TypeDefEnum.Sequence:
                    {
                        ...
                        nodeType = new NodeTypeSequence()
                        {
                            ...
                            TypeParams = typeParams,
                            TypeDef = typeDefValue,
                            TypeId = typeDef.TypeParam.Value.Value,
                            Docs = docs
                        };
                    }
                    break;
                ...
                case TypeDefEnum.Tuple:
                    {
                        ...
                        nodeType = new NodeTypeTuple()
                        {
                            ...
                            TypeParams = typeParams,
                            TypeDef = typeDefValue,
                            TypeIds = typeDef.Fields.Value.Select(p => (uint)p.Value.Value).ToArray(),
                            Docs = docs
                        };
                    }
                    break;
                case TypeDefEnum.Primitive:
                    {
                        var typeDef = (TypeDefPrimitive)Enum.Parse(typeof(TypeDefPrimitive), ((Enum<TypeDefPrimitive>)type.Ty.TypeDef.Value2).Value.ToString());
                ...
                case TypeDefEnum.BitSequence:
                    {
                        ...
                        nodeType = new NodeTypeBitSequence()
                        {
                            ...
                            TypeParams = typeParams,
                            TypeDef = typeDefValue,
                            TypeIdOrder = typeDef.BitOrderType.Value.Value,
                            TypeIdStore = typeDef.BitStoreType.Value.Value,
                            Docs = docs
                        };
                    }
                    break;
        ...
        foreach (var module in modules)
        {
                ...
                for (int i = 0; i < storage.Entries.Value.Length; i++)
                {
                    ...
                    switch (entry.StorageType.Value)
                    {
                        case Storage.Type.Plain:
                            palletModule.Storage.Entries[i].TypeMap = (((TType)entry.StorageType.Value2).Value.Value, null);
                            break;
                        case Storage.Type.Map:
                            ...
                            palletModule.Storage.Entries[i].TypeMap = (0, new TypeMap()
                            {
                                ...
                                Key = (uint)typeMap.Key.Value.Value,
                                Value = (uint)typeMap.Value.Value.Value
                            });
                            break;
                        default:
            ...
            for (int i = 0; i < constants.Length; i++)
            {
                ...
                palletModule.Constants[i] = new PalletConstant()
                {
                    ...
                    TypeId = (uint)constant.ConstantType.Value.Value,
    ...
    private static ExtrinsicMetadata CreateExtrinsic(ExtrinsicMetadataStruct extrinsic)
    {
        ...
        {
            TypeId = (uint)extrinsic.ExtrinsicType.Value.Value,
            ...
            SignedExtensions = extrinsic.SignedExtensions.Value.Select(p => new SignedExtensionMetadata()
            {
                ...
                SignedExtType = (uint)p.SignedExtType.Value.Value,
                AddSignedExtType = (uint)p.AddSignedExtType.Value.Value,
            }).ToArray()
        };
    }
