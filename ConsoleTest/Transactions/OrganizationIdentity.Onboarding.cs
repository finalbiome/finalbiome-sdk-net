...
namespace FinalBiome.Sdk.Transactions
{
    public partial class OrganizationIdentity
    {
        ...
        public async Task<string?> Onboarding(Ajuna.NetApi.Model.Types.Account account, FinalBiome.Sdk.SpCore.Crypto.AccountId32 organizationId, Func<string, ExtrinsicStatus, Task>? subscription = null, CancellationToken? token = null)
        {
            BaseTuple<FinalBiome.Sdk.SpCore.Crypto.AccountId32> data = new BaseTuple<FinalBiome.Sdk.SpCore.Crypto.AccountId32>();
            data.Create(organizationId);
            byte[] parameters = data.Encode();
            Method method = new Method(9, 4, parameters);
