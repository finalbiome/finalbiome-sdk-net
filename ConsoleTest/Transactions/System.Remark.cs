...
namespace FinalBiome.Sdk.Transactions
{
    public partial class System
    {
        ...
        public async Task<string?> Remark(Ajuna.NetApi.Model.Types.Account account, FinalBiome.Sdk.VecU8 remark, Func<string, ExtrinsicStatus, Task>? subscription = null, CancellationToken? token = null)
        {
            BaseTuple<FinalBiome.Sdk.VecU8> data = new BaseTuple<FinalBiome.Sdk.VecU8>();
            data.Create(remark);
            byte[] parameters = data.Encode();
            Method method = new Method(0, 1, parameters);
