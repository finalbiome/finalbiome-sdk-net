    ...
    public class ParsedType
    {
        ...
        public (string, string) CanonicalName
        {
            get
            {
                ...
                // if Namespace defined, du not change it
                if (Namespace is not null) return (Namespace, typeAsPath.Last());

                if (typeAsPath.Length == 1) return (TypeGenerator.RootNamespace, typeAsPath[0]);
                else
                {
                    string Namespace = $"{TypeGenerator.RootNamespace}." + String.Join(".", typeAsPath.Take(typeAsPath.Length - 1));
